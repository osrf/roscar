...
static uint8_t g_usb_device_descriptor[18];
...
void usb_init()
{
  ...
  pin_set_alternate_function(GPIOA, PORTA_USB_VBUS_PIN, 10); // usb is AF10
  ...
  delay_ms(1);
  // this next operation will also mask out all USB interrupts while
  // we're messing with its global configuration and stuff
  USB_OTG_FS->GAHBCFG = USB_OTG_GAHBCFG_TXFELVL ; // interrupt only when empty
  /*
  USB_OTG_FS->GUSBCFG |= USB_OTG_GUSBCFG_FDMOD  | // set device mode
                         (10 << 10)             | // set turnaround to 10 clk
  ...
  *((uint32_t *)(USB_OTG_FS_PERIPH_BASE + USB_OTG_PCGCCTL_BASE)) = 0;
  ...
  //printf("USB GINTSTS = 0x%08x\n", (unsigned)USB_OTG_FS->GINTSTS);
  g_usbd->DCTL |=  USB_OTG_DCTL_SDIS;
  delay_ms(1);
  g_usbd->DCTL &= ~USB_OTG_DCTL_SDIS;
}
